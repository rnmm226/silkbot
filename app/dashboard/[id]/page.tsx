...
function ThinkingBlock({ summary }: { summary: ThinkingSummary }) {
  ...
  return (
    <div className="mb-2.5 rounded-2xl overflow-hidden border border-[var(--color-border,#2a2520)]"
      style={{ background: 'color-mix(in oklch, var(--color-primary,#c4956a) 4%, transparent)' }}>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-4 py-2.5 text-left group transition-colors hover:bg-white/[0.02]"
      >
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <span className="text-xs font-medium" style={{ color: 'var(--color-muted-foreground,#8a7f72)' }}>
            <span style={{ color: 'var(--color-primary,#c4956a)', fontVariantNumeric: 'tabular-nums' }}>
              {summary.chunks_analyzed}
            </span> passages analysés ·{' '}
            <span style={{ color: 'var(--color-primary,#c4956a)' }}>
              {summary.chunks_retained}
            </span> retenus ·{' '}
            <span style={{ color: 'var(--color-primary,#c4956a)' }}>
              {summary.documents_retained.length}
            </span> document{summary.documents_retained.length !== 1 ? 's' : ''} utilisé{summary.documents_retained.length !== 1 ? 's' : ''}
          </span>
        </div>
        {open
      ...
      {open && (
        <div className="px-4 pb-3 pt-0 border-t" style={{ borderColor: 'color-mix(in oklch, var(--color-primary,#c4956a) 15%, transparent)' }}>
          <div className="flex flex-col gap-1.5 mt-2.5">
            {summary.steps.map((step, i) => (
              <div key={i} className="flex items-start gap-2.5 text-xs" style={{ color: 'var(--color-muted-foreground,#8a7f72)' }}>
                <span className="mt-1.5 w-1 h-1 rounded-full shrink-0" style={{ background: 'var(--color-primary,#c4956a)', opacity: 0.5 }} />
                {step}
              </div>
            ))}
          </div>
          {summary.documents_retained.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3 pt-2.5" style={{ borderTop: '1px solid color-mix(in oklch, var(--color-primary,#c4956a) 12%, transparent)' }}>
...
function SourcesBlock({ sources, onSourceClick }: { sources: UsedSource[]; onSourceClick: (s: UsedSource) => void }) {
  ...
  return (
    <div className="mt-4 pt-3.5" style={{ borderTop: '1px solid color-mix(in oklch, currentColor 10%, transparent)' }}>
      ...
      <div className="flex flex-wrap gap-1.5">
        {sources.map((source) => (
          ...
          >
            ...
            {source.page && (
              <span style={{ opacity: 0.6, fontSize: '10px' }}>· p.{source.page}</span>
            )}
          </button>
        ))}
      </div>
...
export default function DashboardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Cycle des étapes de chargement
  ...
  if (isPending) return (
    <div className="flex items-center justify-center h-screen" style={{ background: 'var(--color-background,#0f0e0c)' }}>
      <div className="flex flex-col items-center gap-3">
        ...
        <span className="text-xs font-light tracking-widest" style={{ color: 'var(--color-muted-foreground,#8a7f72)' }}>
          CHARGEMENT
        </span>
      </div>
    </div>
      ...
      <SidebarInset>
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 px-4"
          ...
          <div className="flex items-center gap-2 ml-auto shrink-0">
            <span className="hidden sm:block text-xs font-light" style={{ color: 'var(--color-muted-foreground,#8a7f72)' }}>
              {data?.user?.name}
            </span>
                ...
                <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
                  {SUGGESTIONS.map((s) => (
                    ...
                    >
                      ...
                      <div className="text-xs font-medium" style={{ color: 'var(--color-foreground,#e8e0d0)' }}>
                        {s.label}
                      </div>
                    </button>
                  ))}
