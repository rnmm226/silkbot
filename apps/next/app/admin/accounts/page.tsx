...
export default function AdminAccountsPage() {
  ...
  // Fonction pour fermer le panneau des détails
  const closeDetails = useCallback(() => {
    setSelectedAccount(null);
  }, []);

  // Fonction pour ouvrir les détails d'un compte
  const openDetails = useCallback((account: AccountItem) => {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={cn(
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            {[
              ...
              { label: "En attente", value: stats.unverified, icon: UserX, color: "text-yellow-500" },
              { label: "Administrateurs", value: stats.admins, icon: Shield, color: "text-purple-500" },
            ].map((stat) => (
              <div
                key={stat.label}
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          ...
                          <DropdownMenuItem onClick={() => copyEmail(acc.email)}>
                            <Mail className="size-4" /> Copier l'email
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => copyId(acc.id)}>
                            <Copy className="size-4" /> Copier l'identifiant
                          </DropdownMenuItem>
                          {!acc.emailVerified && (
                            <DropdownMenuItem onClick={() => void verifyAccount(acc)}>
                              <CheckCircle2 className="size-4" /> Vérifier l'email
                            </DropdownMenuItem>
                          )}
                          <DropdownMenuSeparator />
                          <DropdownMenuItem
        ...
        {/* Panneau latéral - Détails du compte (visible uniquement quand un compte est sélectionné) */}
        {selectedAccount && (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {!selectedAccount.emailVerified && (
                      <Button
                        ...
                        onClick={() => void verifyAccount(selectedAccount)}
                      >
                    ...
                    <Button
                      ...
                      onClick={() => void deleteAccount(selectedAccount)}
                    >
